...
exports.createUser = (req, res) => {
    if (password_schema.validate(req.body.users_password)) {
        if (req.body.users_email) {
            bdd.query(Users.selectUsersByEmail(), [req.body.users_email], function(err, result) {
                if(err){
...
exports.createUserProfil = (req, res) => {
    if (req.body.users_name) {
        bdd.query(Users.selectUsersByUserName(), [req.body.users_name], function (err, result){
            if(err){
                res.status(400).json(error(error.message));
            } else if (result[0] != undefined) {
                return res.status(401).json(error("Ce pseudo est déjà utilisé par un autre membre !"));
            } else {
                bdd.query(Users.createUserProfil(), [req.body.users_name, req.body.users_age, req.body.users_biography, req.params.id], function(err) {
                    if(err){
...
exports.selectOneUser = (req, res) => {
    bdd.query(Users.selectUsersById(), [req.params.id], function (error, result){
        if(error) {
            throw error;
        } else {
